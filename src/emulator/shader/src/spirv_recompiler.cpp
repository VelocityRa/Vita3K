...
static spv::Id get_type_basic(spv::Builder &b, const SceGxmProgramParameter &parameter) {
    ...
    switch (type) {
    // clang-format off
    case SCE_GXM_PARAMETER_TYPE_F16:return b.makeFloatType(NO_HALF_FLOAT ? 32 : 16);
...
static spv::Id create_variable(spv::Builder &b, SpirvShaderParameters &parameters, std::string &name, USSE::RegisterBank reg_type, uint32_t size, spv::Id type) {
    ...
    optional<SpirvVarRegBank &> var_group;
...
// TODO: Refine params
static void generate_shader_body(spv::Builder &b, const SpirvShaderParameters &parameters, const SceGxmProgram &program, emu::SceGxmProgramType program_type, bool force_shader_debug) {
    detail::convert_gxp_usse_to_spirv(b, program, parameters, force_shader_debug);
    //if (program_type == emu::SceGxmProgramType::Fragment) {
    //    const auto frag_color_output = parameters.fragment_outputs.vars.at(0);
    ...
    //    const spv::Id float_0_const = b.makeFloatConstant(0.f);
    //    const spv::Id float_1_const = b.makeFloatConstant(1.f);

    //    const spv::Id vec4_blue_const = b.makeCompositeConstant(frag_color_type,
    //        { float_0_const, float_0_const, float_1_const, float_1_const });

    //    b.createStore(vec4_blue_const, frag_color_var);
    //} else if (program_type == emu::SceGxmProgramType::Vertex) {
    //    // TODO: move to frag too
    //    detail::convert_gxp_usse_to_spirv(b, program, parameters);
    //}
}
...
static SpirvCode convert_gxp_to_spirv(const SceGxmProgram &program, const std::string &shader_hash, bool force_shader_debug) {
    ...
    generate_shader_body(b, parameters, program, program_type, force_shader_debug);
...
//    SpirvVarRegBank test_bank;
//    test_bank.push({ 0, 1 }, 4);
//    test_bank.push({ 2, 3 }, 2);
//    test_bank.push({ 4, 5 }, 8);
//    test_bank.push({ 6, 7 }, 4);
//    SpirvReg test_reg;
//    uint32_t comp_offset;
//    test_bank.find_reg_at(7, test_reg, comp_offset);
